...
class Operations(Scene):
    ...
    def create_polar(self, r, angle, axes, label, colors=[BLUE, YELLOW], label_dir=UR):
        ...
        pobjects = create_polarobj(r=r, angle=angle, point=point, axes=axes, label=label, colors=colors, label_dir=label_dir)
        ...
        self.wait()
        self.play(Write(pobjects["pcoords"]))
        self.wait(1)
        return pobjects
    ...
    def add_objects(self, obj1, obj2, axes):
        ...
        self.remove(line2)
        self.wait(2)
        obj3 = self.create_polar(r=np.sqrt(dest[0]**2+dest[1]**2), angle=np.arctan(dest[1]/dest[0]), axes=axes, label=["C", "\\angle\\alpha"], colors=[DARK_BROWN, GRAY], label_dir=UL)
        self.remove(obj3["pcoords"])
        self.add(MathTex(f"(C, \\angle \\alpha)").set_color(RED).next_to(obj3["point"], UL))
        self.wait(5)
...
class Exponentiation(Scene):
    def construct(self):
        ...
        point2 = Dot().move_to(polar_plane.polar_to_point(1, PI/6)).set_color(YELLOW)
        ...
        new_line2 = MathTex(" = ", "1\\angle n\\phi").next_to(expeqn, DOWN).set_color_by_gradient(BLUE, GREEN)
        self.play(Create(point2), Create(pobjects1["line"]), run_time=1)
        self.play(point2.animate.move_to(polar_plane.polar_to_point(1, PI/6)))
        # self.wait()
        path = ParametricFunction(lambda t: np.array(polar_plane.polar_to_point(1, t)), t_range=[angles[0], angles[-1]])
        pow_angle = Arc(0.5, 0, PI, arc_center=polar_plane.polar_to_point(0, 0), color=YELLOW, stroke_width=6)
        pow_label = MathTex("6\\phi").next_to(pow_angle, UP)
        self.play(MoveAlongPath(point2, path), Create(pow_angle), run_time=2)
        self.wait()
        self.play(Write(pow_label))
        self.play(Write(pobjects1["pcoords"].next_to(point2, UL)))
        self.wait(4)
        self.play(Uncreate(VGroup(brace, b_label1, b_label)))
        self.play(Write(new_line2))
        self.wait(6)
        self.play(*[FadeOut(mobj) for mobj in self.mobjects if mobj != polar_plane])
        root_eqn = MathTex("\\sqrt[n]{1\\angle\\theta}", "= (1\\angle\\theta)^\\frac{1}{n}", "= 1\\angle\\theta/n").to_edge(UP).set_color_by_gradient(BLUE, GREEN)
        point3 = Dot().move_to(polar_plane.polar_to_point(0, 0)).set_color(YELLOW)
        pobjects2 = create_polarobj(r=6, angle=5*PI/6, point=point3, axes=axes, label=["1", "\\angle\\theta"], colors=[YELLOW, GREEN], label_dir=UL, origin=polar_plane.polar_to_point(0, 0))
        
        root_angle = Arc(0.5, 0, 5*PI/6, arc_center=polar_plane.polar_to_point(0, 0), color=YELLOW, stroke_width=6)
        root_label = MathTex("\\theta").next_to(root_angle, UR)
        self.play(Write(root_eqn[0]))
        self.play(Create(point3), Create(pobjects2["line"]), run_time=1)
        self.play(point3.animate.move_to(polar_plane.polar_to_point(1, 0)))
        # self.wait()
        path2 = ParametricFunction(lambda t: np.array(polar_plane.polar_to_point(1, t)), t_range=[0, 5*PI/6])
        self.play(MoveAlongPath(point3, path2), Create(root_angle), run_time=2)
        self.play(Write(pobjects2["pcoords"].next_to(point3, UL)), Write(root_label))
        self.wait(4)
        self.play(Write(root_eqn[1]))
        self.wait()
        angles = [5*PI/6, 5*PI/12, 5*PI/18]
        lines = [Line(start=polar_plane.polar_to_point(0, 0), end=polar_plane.polar_to_point(1.1, angle)) for angle in angles]
        self.play(
            *[Create(line) for line in lines]
        )
        self.play(Write(root_eqn[2]))
        point2 = Dot().move_to(polar_plane.polar_to_point(1, 5*PI/6)).set_color(YELLOW)
        pobjects1 = create_polarobj(r=6, angle=5*PI/18, point=point2, axes=axes, label=["1^\\frac{1}{3}", "\\theta/3"], colors=[YELLOW, GREEN], label_dir=UR, origin=polar_plane.polar_to_point(0, 0))
        # pobjects1["point"] = point2
        self.play(Create(point2), Create(pobjects1["line"]), run_time=1)
        self.play(point2.animate.move_to(polar_plane.polar_to_point(1, 5*PI/6)))
        # self.wait()
        path = ParametricFunction(lambda t: np.array(polar_plane.polar_to_point(1, t)), t_range=[angles[-1], angles[0]])
        path.reverse_points()
        pow_angle = Arc(0.5, 0, 5*PI/18, arc_center=polar_plane.polar_to_point(0, 0), color=YELLOW, stroke_width=6)
        pow_label = MathTex("\\theta/3").scale(0.7).next_to(pow_angle, RIGHT + UP*0.05)
        self.play(MoveAlongPath(point2, path), Create(pow_angle), run_time=2)
        self.wait()
        self.play(Write(pow_label))
        self.play(Write(pobjects1["pcoords"].next_to(point2, UR)))
        self.wait(8)
        finale_text = MathTex("\\sqrt{1\\angle\\pi} = ", "1\\angle\\pi/2").to_edge(UP).set_color_by_gradient(BLUE, GREEN)
        finale_text_replaced = MathTex("\\sqrt{-1} = ", "1\\angle\\pi/2").to_edge(UP).set_color_by_gradient(BLUE, GREEN)
        self.play(*[FadeOut(mobj) for mobj in self.mobjects if mobj != polar_plane])
        point2 = Dot().move_to(polar_plane.polar_to_point(0, 0)).set_color(YELLOW)
        pobjects1 = create_polarobj(r=6, angle=PI, point=point2, axes=axes, label=["1", "\\pi"], colors=[YELLOW, GREEN], label_dir=LEFT, origin=polar_plane.polar_to_point(0, 0))
        # pobjects1["point"] = point2
        self.play(Create(point2), Create(pobjects1["line"]), Write(finale_text[0]), run_time=1)
        self.play(point2.animate.move_to(polar_plane.polar_to_point(1, 0)))
        # self.wait()
        path = ParametricFunction(lambda t: np.array(polar_plane.polar_to_point(1, t)), t_range=[0, PI])
        pow_angle = Arc(0.5, 0, PI, arc_center=polar_plane.polar_to_point(0, 0), color=YELLOW, stroke_width=6)
        pow_label = MathTex("\\pi").next_to(pow_angle, UL)
        self.play(MoveAlongPath(point2, path), Create(pow_angle), run_time=2)
        self.wait()
        self.play(Write(pow_label))
        self.play(Write(pobjects1["pcoords"].next_to(point2, UL)))
        self.wait(4)
        pi2line = Line(start=polar_plane.polar_to_point(0, 0), end=polar_plane.polar_to_point(1, PI/2))
        line_label = MathTex("1\\angle\\pi/2").next_to(pi2line, UP + RIGHT*0.05)
        pow_angle2 = Arc(0.2, 0, PI/2, arc_center=polar_plane.polar_to_point(0, 0), color=YELLOW, stroke_width=6)
        self.play(Write(finale_text[1]), Create(pi2line), Write(line_label), Create(pow_angle2))
        self.wait(4)
        self.play(Transform(finale_text, finale_text_replaced))
        self.wait(4)
        self.play(*[FadeOut(mobj) for mobj in self.mobjects if mobj != finale_text])
        self.wait()
        self.play(finale_text.animate.move_to(ORIGIN).scale(2))
